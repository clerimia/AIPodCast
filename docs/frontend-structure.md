...
- `tool:start/tool:end` → 写运行态 store（状态条只留当前窗口的工具 + 累计步数；完成的调用随下一处 `message:end` 归属上一条 assistant 气泡，Task 块实时出现）；
- `delta` / `message:end` → 流式气泡（运行态 store）；
- `done` → 关流 + 最终 `invalidateQueries(['script'])` + 失效 writerHistory 对齐回放 + 恢复输入；
- `error` → toast + 关流。
